...
clear_chars_in_line(CPUCell *cpu_cells, GPUCell *gpu_cells, index_type xnum, char_type ch) {
    ...
    if (ch) {
        ...
        for (index_type i = 0; i < xnum; i++) {
            cpu_cells[i] = c;
            gpu_cells[i].attrs = empty;
        }
    }
}
...
#error This implementation is incorrect for BLANK_CHAR != 0
    ...
    for (index_type i = 0; i < line->xnum; i++)
        if (line->cpu_cells[i].ch_and_idx) return false;
    return true;
}
...
bool line_as_ansi(Line *self, ANSILineState *s, index_type start_at, index_type stop_before, char_type prefix_char, bool skip_multiline_non_zero_lines)
...
void add_marks_to_line(PyObject *marker, Line *line, ANSIBuf *buf, unsigned long line_number);
